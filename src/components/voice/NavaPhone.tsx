...
export default function NavaPhone() {
  ...
  return (
    <section className="overflow-hidden rounded-[20px] border border-white/10 bg-[radial-gradient(circle_at_top_right,_rgba(31,210,180,0.12),_transparent_38%),linear-gradient(145deg,_rgba(15,27,49,0.98),_rgba(8,13,28,0.98))] p-3 shadow-2xl shadow-black/20 sm:p-4">
      ...
      <div className="mx-auto mt-2 grid max-w-[260px] grid-cols-3 gap-1">
        {DIAL_KEYS.map((key) => (
          <button
            key={key.value}
            type="button"
            onClick={() => appendDigit(key.value)}
            className="group flex h-10 flex-col items-center justify-center rounded-xl border border-white/10 bg-white/[0.045] text-white transition hover:border-cyan-300/40 hover:bg-cyan-300/10 active:scale-95"
      ...
      <div className="mx-auto mt-1 flex max-w-[260px] items-center justify-between gap-2">
        <button
          type="button"
          onClick={clearNumber}
          className="rounded-lg px-2.5 py-1.5 text-[11px] font-bold text-slate-500 transition hover:bg-white/5 hover:text-white"
        >
          Clear
        </button>
