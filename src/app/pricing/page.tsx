...
export const metadata: Metadata = {
  title: "Pricing Â· FredOTP virtual number rentals",
  description:
        ...
        <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            <PriceStat
              label="Cheapest code"
              value={usd(cheapest)}
              note={`${cheapestService?.name ?? "Discord"} Â· ${cheapestCountry?.flag ?? ""} ${cheapestCountry?.name ?? ""} pool`}
            />
            <PriceStat label="Average code" value={usd(average)} note={`${catalog.offers.length} live price points`} />
            <PriceStat label="Naira rate" value={`â‚¦${USD_TO_NGN.toLocaleString()} / $1`} note="Applied to all displayed prices" />
          </div>
          ...
          <div className="card mt-10 grid gap-6 p-6 md:grid-cols-3">
            <div>
              <h3 className="text-sm font-bold text-white">ðŸ’³ Payment methods</h3>
            ...
            <div>
              <h3 className="text-sm font-bold text-white">â†©ï¸ Refund policy</h3>
              <p className="mt-2 text-sm text-slate-400">
